...
- **库存预警**：结合当前库存与预测销量，判断“库存不足/正常/过高”，给出建议补货量。
...
| 成员 | 角色         | 主要任务                                                         | 个人产出                                 |
| ---- | ------------ | ---------------------------------------------------------------- | ---------------------------------------- |
| 1号  | 项目经理    | 需求收敛、任务分配、进度管理、文档汇总、PPT与汇报组织            | 项目管理文档、甘特图、分工说明、答辩串词 |
| 2号  | 数据负责人   | 整理电商销售数据、数据清洗、字段字典、数据质量检查               | 数据集、数据说明、清洗代码、质量报告     |
| 3号  | 算法负责人A  | 基线模型（移动平均/Prophet/SARIMAX）、回测与误差分析             | 基线模型代码、回测结果、误差图           |
| 4号  | 算法负责人B  | LightGBM主模型、特征工程、误差评价、库存预警规则                 | 主模型代码、特征工程代码、预测结果表     |
| 5号  | 后端负责人   | FastAPI接口、模型调用、数据读取、接口文档与后端联调              | 后端代码、接口文档、运行说明             |
| 6号  | 前端/测试负责人 | 页面实现、图表交互、测试用例设计、系统截图与录屏                 | 前端页面、测试报告、截图、演示视频素材   |

> 详细分工及任务进度请查看 `docs/management/team_division.md`
